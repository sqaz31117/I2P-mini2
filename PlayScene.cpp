...
int PlayScene::MapTowerID[13][20] = {-1};
	...
	for (int i = 0; i < SpeedMult; i++) {
		...
		if (enemyWaveData.empty()) {
			if (EnemyGroup->GetObjects().empty()) {
				...
				// TODO 6 which crashes the game when you win. Try to find it.
				// delete TileMapGroup;
	...
	if (button & 1) {
		if (mapState[y][x] != TILE_OCCUPIED) {
			...
			// shifter
				...
				int tmp = MapTowerID[y][x];
				MapTowerID[y][x] = -1;
				mapState[y][x] = TILE_FLOOR;
				preview = nullptr;
				if (tmp == 0) {
					preview = new PlugGunTurret(x, y);
				}
				else if (tmp == 1) {
					preview = new PlugGunTurret2(x, y);
				}
				else if (tmp == 2) {
					preview = new PlugGunTurret3(x, y);
				}
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		// Earn money : money += 10000;
			...
			bool check = false;
			int i = 0;
			for (std::list<int>::iterator it = keyStrokes.begin(); it != keyStrokes.end(); it++, i++) {
				if((*it) != code[i]){
					check = true;
					break;
				}
...
void PlayScene::Hit() {
	// TODO 6 which doesn't update the player's life correctly when getting the first attack. Try to find it.
...
void PlayScene::ReadMap() {
	...
	// mapTower = std::vector<std::vector<int>>(MapHeight, std::vector<int>(MapWidth, -1));
	for (int i = 0; i < MapHeight; i++) {
		for (int j = 0; j < MapWidth; j++) {
...
void PlayScene::ConstructButton(int id, std::string sprite, int price) {
	...
	if (id <= 2) {
		...
		btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1294 + (id - 3) * 76, 212, 0, 0, 0, 0),
		Engine::Sprite(sprite, 1294 + (id - 3) * 76, 212 - 8, 0, 0, 0, 0)
